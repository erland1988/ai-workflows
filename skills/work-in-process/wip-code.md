...
3. 按 plan.md 执行编码（模式 A 逐 Step / 模式 B 子代理整模块）
4. Step 失败 → 暂停等待人工介入。plan 与代码有出入 → 先回写 plan 再继续
5. 格式扫尾，确保新增代码与项目风格一致
6. **合并**：feature 分支合并回基分支，清理 worktree
7. **更新 ledger.md**：记录模块级事件、合并信息、关键决策
